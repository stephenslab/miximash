#' @importFrom ashr lik_normal lik_t set_data normalmix
#' 
#' @export
miximash <- function (betahat, sebetahat, mixcompdist = "normal",
                      optmethod = "w_mixEM",df = NULL,nullweight = 10,
                      pointmass = TRUE,mixsd = NULL,gridmult = sqrt(2),
                      outputlevel = 2,g = NULL,fixg = FALSE,mode = 0,
                      alpha = 0,grange = c(-Inf,Inf),control = list(),
                      lik = NULL,weights = NULL,pi_thresh = 1e-10,K = 1,
                      BIC = TRUE,mess = TRUE) {
  ...
  if (is.null(mixsd)) {
    mixsd = ashr:::autoselect.mixsd(data_v,gridmult,mode,grange,mixcompdist)
  }
  if(pointmass){
    mixsd = c(0,mixsd)
  }
  ...
  w = ashr:::initpi(L,n,null.comp)
  if (mixcompdist == "normal")
    g <- normalmix(w,rep(mode,L),mixsd)
...
#' @importFrom ashr mixEM w_mixEM
#' @importFrom MCMCpack rdirichlet
gcmfit <- function (matrix_llik,K = 1,max.iter = 300,tol = 1e-4,w_init,
                    pi_thresh,mess = TRUE) {
  ...
  W = list()
  for(r in 1:R){
    W[[r]] = rdirichlet(K,alpha=rep(1,L))
    W[[r]][1,] = w_init
  }
  ...
  for(i.iter in 1:max.iter) {
    if(mess){
      if((i.iter%%5) == 0) {
        print(paste("We have run the first ",
                    i.iter, " iterations for K=", K,sep=""))
      }
    }
    ...
    p.new = ashr:::normalize(fit.p$pihat + 1e-8)
    ...
    for(j in 1:n){
      clustlike[j,] = ashr:::normalize(clustlike[j,])
    }
    W.new = W
    for(k in 1:K){
      for(r in 1:R){
        if(sum(clustlike[,k])>0){
          ww = ashr:::normalize((clustlike[,k]))
  ...
  loglike = ashr:::penloglik(p,matrix_lik2,prior=1)+sum(tempmax)
...
#' @importFrom ashr comp_postprob comp_postmean comp_cdf_post
